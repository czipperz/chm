...
void hash_map_reserve(void* _hash_map, size_t key_size, size_t value_size,
    ...
    if (hash_map->count + extra > hash_map->cap / 2) {
        ...
        new_hash_map._flags = (unsigned char*) calloc(
            (new_hash_map.cap * 2 + CHAR_BIT - 1) / CHAR_BIT, 1);
        ...
        for (uint64_t i = 0; i < hash_map->cap; ++i) {
            if (hash_map_is_present(hash_map, i)) {
                hash_map_insert(&new_hash_map, key_size, value_size,
                                &hash_map->keys[i * key_size],
                                hashf(&hash_map->keys[i * key_size]),
                                &hash_map->values[i * value_size]);
            }
        }
...
void* hash_map_get(const void* _hash_map, size_t key_size, size_t value_size,
    ...
    for (;; index = ((index + 1) & (hash_map->cap - 1))) {
        if (hash_map_is_present(hash_map, index)
            && comparef(&hash_map->keys[index * key_size], key) == 0) {
            return &hash_map->values[index * value_size];
        } else if (hash_map_is_present(hash_map, index)
...
void hash_map_insert(void* _hash_map, size_t key_size, size_t value_size,
...
int hash_map_remove(void* _hash_map, size_t key_size, size_t value_size,
    ...
    while (1) {
        ...
        if (hash_map_is_present(hash_map, index)
            && comparef(&hash_map->keys[index * key_size], key) == 0) {
            memcpy(value_out, &hash_map->values[index * value_size],
                   value_size);
